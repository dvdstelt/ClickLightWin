...
public sealed class LowLevelMouseHook : IDisposable
{
    ...
    private bool HandleAnnotation(int msg, int x, int y)
    {
        ...
        // Ctrl+Shift + drag = a permanent annotation (arrow by left, box by right),
        // swallowed so it never reaches the app. Plain Ctrl is deliberately left
        // alone so Ctrl+click (links, multi-select) keeps working.
        var isDown = msg is NativeMethods.WM_LBUTTONDOWN or NativeMethods.WM_RBUTTONDOWN;
        if (!isDown || !AnnotationsEnabled
            || !NativeMethods.IsDown(NativeMethods.VK_CONTROL) || !NativeMethods.IsDown(NativeMethods.VK_SHIFT))
            return false;
